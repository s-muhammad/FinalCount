...
@section('title', 'اخبار')
@section('header', 'مدیریت اخبار')

@section('content')
<div class="bg-white rounded-xl shadow-sm">
    <div class="px-6 py-4 border-b flex items-center justify-between">
        <h3 class="font-bold text-gray-800">لیست اخبار</h3>
        <a href="{{ route('admin.news.create') }}" class="bg-primary text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-light transition flex items-center gap-2">
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">ویژه</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($news as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            @if($item->is_featured)
                                <span class="px-2 py-1 text-xs rounded-full bg-primary/10 text-primary">ویژه</span>
                            @else
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.news.edit', $item) }}" class="p-1.5 hover:bg-gray-100 rounded-lg">
                                ...
                                <form action="{{ route('admin.news.destroy', $item) }}" method="POST" onsubmit="return confirm('آیا مطمئن هستید؟')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-gray-500">خبری ثبت نشده</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
