...
@section('header', 'ایجاد پیام جدید')
            ...
            <div class="bg-white rounded-xl shadow-sm p-6" x-data="{ lang: 'fa' }">
                ...
                <div class="space-y-4" x-show="lang === 'ar'" dir="rtl">
                    <div>
                        ...
                        <input type="text" name="title_ar" value="{{ old('title_ar') }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-primary text-right">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">الملخص</label>
                        <textarea name="summary_ar" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-primary text-right">{{ old('summary_ar') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">النص</label>
                        <textarea name="body_ar" rows="10" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-primary text-right">{{ old('body_ar') }}</textarea>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="type" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-primary">
                            <option value="speech" {{ old('type') === 'speech' ? 'selected' : '' }}>سخنرانی</option>
                            <option value="message" {{ old('type') === 'message' ? 'selected' : '' }}>پیام</option>
                            <option value="decree" {{ old('type') === 'decree' ? 'selected' : '' }}>حکم</option>
                        </select>
                    </div>
                    <div>
