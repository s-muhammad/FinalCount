...
@section('header', 'مدیریت گفتگوها')

@section('content')
<div class="bg-white rounded-xl shadow-sm">
    <div class="px-6 py-4 border-b flex items-center justify-between">
        <h3 class="font-bold text-gray-800">لیست گفتگوها</h3>
        <a href="{{ route('admin.interviews.create') }}" class="bg-primary text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-light transition flex items-center gap-2">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($interviews as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500">{{ $item->guest_name ?? '-' }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.interviews.edit', $item) }}" class="p-1.5 hover:bg-gray-100 rounded-lg">
                                ...
                                <form action="{{ route('admin.interviews.destroy', $item) }}" method="POST" onsubmit="return confirm('آیا مطمئن هستید؟')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-gray-500">گفتگویی ثبت نشده</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
