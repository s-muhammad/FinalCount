...
@section('title', 'نقل‌قول‌ها')
@section('header', 'مدیریت نقل‌قول‌ها')

@section('content')
<div class="bg-white rounded-xl shadow-sm">
    <div class="px-6 py-4 border-b flex items-center justify-between">
        <h3 class="font-bold text-gray-800">لیست نقل‌قول‌ها</h3>
        <a href="{{ route('admin.quotes.create') }}" class="bg-primary text-white px-4 py-2 rounded-lg text-sm hover:bg-primary-light transition flex items-center gap-2">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($quotes as $item)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 text-sm text-gray-800 max-w-md truncate">{{ $item->body }}</td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.quotes.edit', $item) }}" class="p-1.5 hover:bg-gray-100 rounded-lg">
                                ...
                                <form action="{{ route('admin.quotes.destroy', $item) }}" method="POST" onsubmit="return confirm('آیا مطمئن هستید؟')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-gray-500">نقل‌قولی ثبت نشده</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
