...
@section('main')
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6 md:p-10">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-0 overflow-hidden">
            <form action="{{ route('admin.blog.update',$blog->id) }}" method="POST" enctype="multipart/form-data" class="relative">
                @csrf
                @method('put')
                ...
                <div class="hidden peer-checked/fa:block p-8 space-y-6 animate-fade-in">
                    ...
                    <div>
                        ...
                        <input type="text" name="title[fa]" value="{{ $blog->getTranslation('title', 'fa') }}"
                    ...
                    <div>
                        ...
                        >{{ $blog->getTranslation('summary', 'fa') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition duration-300">{{ $blog->getTranslation('body', 'fa') }}</textarea>
                    </div>
                    ...
                    <div class="select2-blue">
                        ...
                        <select name="tags[fa][]" multiple="multiple" id="tags-fa" class="w-full select2-input">
                            @foreach($faTags as $tag)
                                <option value="{{ $tag->name }}"
                                    {{ (in_array($tag->name, old('tags.fa', $blog->tagsWithType('fa')->pluck('name')->toArray()))) ? 'selected' : '' }}>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="hidden peer-checked/ar:block p-8 space-y-6 animate-fade-in">
                    ...
                    <div>
                        ...
                        <input type="text" name="title[ar]" value="{{ $blog->getTranslation('title', 'ar') }}"
                    ...
                    <div>
                        ...
                        >{{ $blog->getTranslation('summary', 'ar') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        focus:border-purple-500 focus:ring-4 focus:ring-purple-100 transition duration-300">{{ $blog->getTranslation('body', 'ar') }}</textarea>
                    </div>
                    ...
                    <div class="select2-purple">
                        ...
                        <select name="tags[ar][]" multiple="multiple" id="tags-ar" class="w-full select2-input">
                            @foreach($arTags as $tag)
                                <option value="{{ $tag->name }}"
                                    {{ (in_array($tag->name, old('tags.ar', $blog->tagsWithType('ar')->pluck('name')->toArray()))) ? 'selected' : '' }}>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="hidden peer-checked/en:block p-8 space-y-6 animate-fade-in" dir="ltr">
                    ...
                    <div>
                        ...
                        <input type="text" name="title[en]" value="{{ $blog->getTranslation('title', 'en') }}"
                    ...
                    <div>
                        ...
                        >{{ $blog->getTranslation('summary', 'en') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        focus:border-green-500 focus:ring-4 focus:ring-green-100 transition duration-300 text-left">{{ $blog->getTranslation('body', 'en') }}</textarea>
                    </div>
                    ...
                    <div class="select2-green text-left">
                        ...
                        <select name="tags[en][]" multiple="multiple" id="tags-en" class="w-full select2-input" dir="ltr">
                            @foreach($enTags as $tag)
                                <option value="{{ $tag->name }}"
                                    {{ (in_array($tag->name, old('tags.en', $blog->tagsWithType('en')->pluck('name')->toArray()))) ? 'selected' : '' }}>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="pt-8 border-t border-gray-200 mx-8 mb-8">
                    <div>
                        <label for="image" class="block text-gray-700 text-sm font-medium mb-3">تصویر مقاله</label>
                        <div class="flex items-center space-x-4 space-x-reverse">
                            <div class="w-48 h-32 bg-gray-50 rounded-xl flex items-center justify-center text-gray-400 border-2 border-dashed border-gray-300 shadow-sm">
